...
sed -i "s|BETTER_AUTH_URL=.*|BETTER_AUTH_URL=$DNS|g" .env.production && \
sed -i "s|FRONTEND_URL=.*|FRONTEND_URL=$DNS|g" .env.production && \
./start.sh prod
...
sed -i "s|FRONTEND_URL=.*|FRONTEND_URL=staging.$DNS|g" .env.production && \
./start.sh
