...
echo "Restoring DB..."
cat $FOLDER/dump.sql | docker exec -e PGPASSWORD=$DB_PASS -i $DB_CONTAINER_NAME psql -U $DB_USER -d postgres

echo "Purging kvStore cache..."
docker exec -i prod-kvStore redis-cli FLUSHALL
